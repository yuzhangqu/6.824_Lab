    ...
    unsigned int xid_rep, char **b, int *sz)
{
  ...
  std::list<reply_t>::iterator it2 = it->second.begin(); 
  ...
  if (!it->second.empty())
  {
    unsigned int leastId = it->second.front().xid;
    for (it2 = it->second.begin(); it2 != it->second.end(); it2++)
    {
      leastId = (leastId > it2->xid ? it2->xid : leastId);
    }
    if (xid <= leastId)
    {
      ret = FORGOTTEN;
    ...
    {
      it2 = it->second.begin();
      while (it2 != it->second.end() && it2->xid != xid)
      {
        it2++;
      }
  ...
  {
    reply_t currep(xid);
    it->second.push_back(currep);
  }
  //printf("check... client: %u, xid: %d, xid_rep: %d, ret: %d\n", clt_nonce, xid, xid_rep, ret);
  return ret;
}
