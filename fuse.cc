...
fuseserver_write(fuse_req_t req, fuse_ino_t ino,
        ...
        struct fuse_file_info *fi)
{
    printf("======fuseserver_write=======%ld\n", time(NULL));//TODO:DELETE IT
    /*
     * your lab2 code goes here.
...
fuseserver_createhelper(fuse_ino_t parent, const char *name,
        mode_t mode, struct fuse_entry_param *e, uint32_t type)
{
     ...
     */
    int ret;
    yfs_client::inum tmpno;
    //yfs->lookup(parent, name, found, tmpno);
    //if (found)
    //{
    //    return yfs_client::EXIST;
    //}
    if ((ret = yfs->create(parent, name, mode, tmpno, type)) != yfs_client::OK)
    {
        return ret;
    }
    getattr(tmpno, e->attr);
    e->ino = tmpno;
    return yfs_client::OK;
}
        ...
        mode_t mode, struct fuse_file_info *fi)
{
    ...
    yfs_client::status ret;
    if( (ret = fuseserver_createhelper( parent, name, mode, &e, 2)) == yfs_client::OK ) {
...
{
    printf("======fuseserver_lookup======parent: %lu, name: %s\n", parent, name);//TODO:DELETE IT
    struct fuse_entry_param e;
    // In yfs, timeouts are always set to 0.0, and generations are always set to 0
    e.attr_timeout = 0.0;
    e.entry_timeout = 0.0;
     ...
     */
    yfs_client::inum tmpino;
    yfs->lookup(parent, name, found, tmpino);
    if (found)
    {
        yfs_client::status ret;
        ret = getattr(tmpino, e.attr);//set e.attr
        if (ret != yfs_client::OK)
        {
            fuse_reply_err(req, ENOENT);
        }
        ...
        mode_t mode)
{
     ...
     */
    yfs_client::status ret;
    if ((ret = fuseserver_createhelper(parent, name, mode, &e, 1)) == yfs_client::OK)
    {
        fuse_reply_entry(req, &e);
    }
    else
